...
using namespace std;
	...
	void shift(auto arr[], int q, bool dir=0, int first=0, int last=-1){ // 0=Left 1=Right
		if(last==-1){last=q-1;};
		if(dir==0){
			for(int i=first+1; i<=last; i++){arr[i-1]=arr[i];};
		}else{
			for(int i=last; i>first; i--){arr[i]=arr[i-1];};
		};
	}
	...
	void wait(int sec, bool show=false, bool reverse=false){
		time_t s=time(NULL); int passed=0;
		while(time(NULL)<s+sec){
			if(time(NULL)-s!=passed && show){
				passed=time(NULL)-s;
				if(!reverse){cout<<passed<<endl;}
				 else{cout<<sec-passed<<endl;};
			};
		};
	}
#endif
